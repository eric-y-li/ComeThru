...
def index():
    ...
    db.execute("DELETE from events WHERE julianday(CURRENT_TIMESTAMP)-julianday(timecreated) > 1")
...
def location():
    ...
    if request.method == "POST":
        ...
        # Return response to ajax call as a redirect to / route
...
def addfriend():
    """Add a friend"""
    # If it's a GET render the appropriate html
...
def requests():
    ...
    # Rows is an array of names of users that sent friend requests to the user in session
    rows = db.execute("SELECT sender_username FROM requests WHERE recipient=:recipient", recipient=session["user_id"])

    # Display requests page if it's get
    if request.method == "GET":
        return render_template("request.html", rows = rows)
    # If the user clicks accept or decline
    else:
        # choice is a string concatenating "Accept" or "Decline" with the associated username
