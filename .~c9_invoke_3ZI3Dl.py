...
def form():
    ...
    if request.method == "POST":
        ...
        if privacy == "private":
            isprivate = "T"
        # Insert all the input into the events database including the user ID of the poster
        result = db.execute("INSERT INTO events (eventname, starttime, endtime, description, eventhost, userid, privacy, timecreated) VALUES (:eventname, :starttime, :endtime, :description, :eventhost, :userid, :privacy, DATETIME(CURRENT_TIMESTAMP, 'localtime'))",
                            eventname=request.form.get("eventName"), starttime=request.form.get("starttime"), endtime=request.form.get("endtime"),
                            description=request.form.get("description"), eventhost=request.form.get("host"),
                            userid=session["user_id"], privacy = isprivate)
